...
Texture2D ShadowMap;
SamplerComparisonState ShadowSampler; //Comparison을 위해ㅔ Sampler ComparisonState를 사용
...
void ComputeSpotLight(inout MaterialDesc output, float3 normal, float3 wPosition)
{
    ...
    for (uint i = 0; i < SpotLightCount; i++)
    {
        ...
        float att = temp * (1.0f / max(1.0f - SpotLights[i].Intensity, 1e-8f)); //값을 뒤집는다.
...
void NormalMapping(float2 uv, float3 normal, float3 tangent, SamplerState samp)
{
    ...
    float3 T = normalize(tangent - dot(tangent, N) * N); //X , 정규직교화 - 그람슈미트 과정
    float3 B = cross(N, T); //Y
    
    float3x3 TBN = float3x3(T, B, N); //Tangent 공간
...
Texture2D ProjectorMap;     //Projector
...
void PS_Projector(inout float4 color, float4 wvp)
{
    ...
    {
        float4 map = ProjectorMap.Sample(LinearSampler, uvw.xy); //화면공간에서 그림
