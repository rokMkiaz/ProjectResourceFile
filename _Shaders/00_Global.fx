...
struct MeshGeometryOutput
{
    ...
    uint TargetIndex : SV_RenderTargetArrayIndex; //현재 지오메트리 셰이더에서 처리하는 삼각형이 렌더타깃의 Array중에 몇번에 렌더링할지를 결정
};
...
{
    AlphaToCoverageEnable = false; //외곽선을 탐지해서 주변의 색과 자연스럽게 섞이도록 처리
    
    BlendEnable[0] = true;
    SrcBlend[0] = SRC_ALPHA; //Src =그릴색, 
    DestBlend[0] = INV_SRC_ALPHA;//Dest = 그려진 색
    BlendOp[0] = ADD;
    
    SrcBlendAlpha[0] = One; //그릴색에 대한 알파값 1
    DestBlendAlpha[0] = Zero; //받을색에대하 값은 0
    BlendOpAlpha[0] = Add;
...
BlendState AdditiveBlend //두색을 섞어 선명하게 함
{
