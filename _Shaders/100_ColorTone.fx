...
float4 PS_Grayscale(VertexOutput input) : SV_Target //흑백
{
...
float4 PS_Grayscale2(VertexOutput input) : SV_Target  //실제 과학적으로 만들어진 흑백수치
{
	float4 color = float4(DiffuseMap.Sample(LinearSampler, input.Uv));
...
float4 PS_Blur(VertexOutput input) : SV_Target //주변 픽셀의 평균을 이용해 흐리게 보이게 함.
{
...
float4 PS_Wiggle(VertexOutput input) : SV_Target //울렁거림 효과
{
	float2 uv = input.Uv;
