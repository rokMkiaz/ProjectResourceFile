...
float4 PS(MeshOutput input) : SV_Target
{
	...
	if (Selected == 0)
	{
        //Texture(Material.Diffuse, DiffuseMap, input.Uv); //흰색
	}
	else if (Selected == 1)
	{
		NormalMapping(input.Uv, input.Normal, input.Tangent); //흰색+ 노멀
	}
	else if (Selected == 2)
	{
		Texture(Material.Diffuse, DiffuseMap, input.Uv); //택스처
	}
	else if (Selected == 3)
	{
		Texture(Material.Diffuse, DiffuseMap, input.Uv); //텍스처+노멀
